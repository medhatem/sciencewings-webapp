...
export class ContactsService {
  ...
  searchContacts(query: string): Observable<any[]> {
    return this._httpClient
      .get<any[]>('api/apps/contacts/search', {
        params: { query },
      })
      .pipe(
        tap((contacts) => {
  ...
  createContact(): Observable<any> {
    return this.contacts$.pipe(
      take(1),
      switchMap((contacts) =>
        this._httpClient.post<any>('api/apps/contacts/contact', {}).pipe(
          map((newContact) => {
  ...
  createTag(tag: any): Observable<any> {
    return this.tags$.pipe(
      take(1),
      switchMap((tags) =>
        this._httpClient.post<any>('api/apps/contacts/tag', { tag }).pipe(
          map((newTag) => {
  ...
  updateTag(id: string, tag: any): Observable<any> {
    return this.tags$.pipe(
      ...
      switchMap((tags) =>
        this._httpClient
          .patch<any>('api/apps/contacts/tag', {
            id,
            tag,
          })
          .pipe(
